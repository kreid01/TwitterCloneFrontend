...
export const Comment:React.FC<Props> = ({ comment }) => {
    ...
    <div className='flex mt-8 mb-4 b border-b-slate-400'>
        ...
        <div className='flex-col pl-4'>  
                <p className=''><Link to={`/${comment.userAt}`}><strong>{comment.userName}</strong></Link>
                <span className='text-gray-500'>@{comment.userAt}·{comment.commentDate.substring(0,7)}</span></p>
            <p>{comment.commentBody}</p>
            <img 
            className='w-80 pt-2 max-h-72 rounded-xl'
            src={comment.commentMedia} alt='' data-testid='media'/>
        </div>
    </div> 
    )
}